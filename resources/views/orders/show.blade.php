                    ...
                    <div class="p-6 text-gray-900">
                        <div class="mb-6">
                            <a href="{{ route('dashboard') }}"
                                class="inline-flex items-center text-gray-600 hover:text-cps-blue transition duration-150">
                        ...
                        <div class="flex justify-between items-center mb-6">
                            <div class="flex items-center gap-4">
                                ...
                                <a href="{{ route('orders.export', $order) }}"
                                    class="bg-green-600 text-white text-sm px-4 py-2 rounded-lg hover:bg-green-700 transition duration-150 flex items-center">
                                ...
                                <a href="{{ route('orders.quotation', $order) }}" target="_blank"
                                    class="bg-gray-800 text-white text-sm px-4 py-2 rounded-lg hover:bg-gray-700 transition duration-150 flex items-center">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z">
                                        </path>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div>
                                ...
                                <p><span class="font-medium">{{ __('Captain Name:') }}</span> {{ $order->captain_name }}
                                </p>
                                <p><span class="font-medium">{{ __('Vessel Name:') }}</span>
                                    {{ $order->vessel_name ?? 'N/A' }}</p>
                        ...
                        <div class="mb-8 bg-gray-50 p-6 rounded-lg border border-gray-200">
                            <h3 class="text-lg font-bold mb-4 text-gray-800">{{ __('Add Product to Order') }}</h3>
                            <form action="{{ route('orders.add-product', $order) }}" method="POST"
                                class="flex gap-4 items-end">
                                @csrf
                                <div class="flex-grow">
                                    <label for="product_id"
                                        class="block text-sm font-medium text-gray-700 mb-1">{{ __('Select Product') }}</label>
                                    <select name="product_id" id="product_id"
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-cps-blue focus:ring-cps-blue select2">
                                        <option value="">{{ __('Select a product...') }}</option>
                                        @foreach($products as $product)
                                            <option value="{{ $product->id }}">{{ $product->name }}
                                                ({{ $product->category->name }}) - ${{ number_format($product->price, 2) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="w-32">
                                    <label for="quantity"
                                        class="block text-sm font-medium text-gray-700 mb-1">{{ __('Quantity') }}</label>
                                    <input type="number" name="quantity" id="quantity" value="1" min="1"
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-cps-blue focus:ring-cps-blue">
                                </div>
                                <button type="submit"
                                    class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition duration-150 font-semibold shadow-md whitespace-nowrap">
                                    {{ __('Add Product') }}
                                </button>
                            </form>
                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">
                                                <form action="{{ route('orders.update-item', [$order, $item]) }}"
                                                    ...
                                                    <input type="number" name="quantity" value="{{ $item->quantity }}"
                                                        min="1"
                                                        class="w-20 rounded-md border-gray-300 shadow-sm focus:border-cps-blue focus:ring-cps-blue text-center text-sm">
                                            ...
                                            <td
                                                ...
                                                <div class="flex items-center justify-end gap-4">
                                                    ...
                                                    <form action="{{ route('orders.remove-item', [$order, $item]) }}"
                                                        method="POST"
                        ...
                        <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
                            ...
                            <form action="{{ route('orders.update', $order) }}" method="POST">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label for="status"
                                            class="block text-sm font-medium text-gray-700 mb-1">{{ __('Order Status') }}</label>
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            <label for="discount_percentage"
                                                class="block text-sm font-medium text-gray-700 mb-1">{{ __('Discount (%)') }}</label>
                                        ...
                                        <div>
                                            <label for="tax"
                                                class="block text-sm font-medium text-gray-700 mb-1">{{ __('Tax ($)') }}</label>
                                        ...
                                        <div>
                                            <label for="shipping_cost"
                                                class="block text-sm font-medium text-gray-700 mb-1">{{ __('Transportation Cost ($)') }}</label>
                                ...
                                <div class="mt-6 border-t border-gray-200 pt-4">
                                    ...
                                    <p class="text-xs text-gray-500 text-right mt-1">
                                        {{ __('(Calculated as: Subtotal - Discount + Tax + Shipping)') }}
                                    </p>
                                </div>
